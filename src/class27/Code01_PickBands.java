...
public class Code01_PickBands {
    ...
    public static void process(int[][] programs,int size,int index,int status,int cost,int rest,int[] map){
        if(rest==0){
            map[status]=Math.min(map[status],cost);
        }else{
            if(index!=size){
                process(programs,size,index+1,status,cost,rest,map);
                int pick = 0|(1 << programs[index][0]) | (1 << programs[index][1]);
                if((pick&status)==0){
                    process(programs,size,index+1,status|pick,cost+programs[index][2],rest-1,map);
                }
            }
        }
